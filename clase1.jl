...
# ╔═╡ df8598e4-3a90-11f1-aa29-3fbdd482c07a
using MAT, LinearAlgebra, Plots



# ╔═╡ 95247924-e6ea-4545-98a6-7829bca340fb
begin
file = matopen("datos.mat")
datos = read(file, "datos_1")
end

# ╔═╡ 866f81f2-c55b-4c89-96ba-0bf96b056066
scatter(datos[:,1],datos[:,2],label="datos",xlabel="x",ylabel="y",aspect_ratio = :equal)

# ╔═╡ b646b7b1-bba5-4e5d-8041-8ca9eb6eb9f7
function def_problem(datos)
	n = length(datos[:,1])
	unos = ones(n)
	A = hcat(2.0.*datos,unos)
	P = datos[:,1].^2 + datos[:,2].^2	
	return A,P
end	

# ╔═╡ 0452dc11-45e0-4981-8e35-2aaf9f01b483
function ajuste_alg(datos)
	# Resolvemos el sistema
	A, P = def_problem(datos)
	AtA = A'*A
	AtP = A'*P
	a,b,c = AtA\AtP

	# Graficamos los datos junto con la solución
	r = sqrt(a^2+b^2+c)
	theta = range(0,2pi,100)
	scatter(datos[:,1],datos[:,2],label="datos",xlabel="x",ylabel="y",legend = (0.4,0.5),aspect_ratio = :equal)
	scatter!(r*cos.(theta).+a,r*sin.(theta).+b,label="Ajuste por cuadrados mínimos")
end	

# ╔═╡ f07fd689-20a9-426e-b9e3-b8e399e78924
ajuste_alg(datos)
